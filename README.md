...
- **Yoga** — instructor-led. Pick 5, 7, 10, or 15 minutes and it walks you through a sequence of poses, each one timed and auto-advancing, with a short countdown between them.
...
- **A 2-second countdown between exercises, in both modes.** A brief "Get ready for &lt;exercise&gt;" countdown (2-1, with its own soft tick and the upcoming move already on screen) gives you a moment to get into position before it narrates the full cue.
...
- **Left/right balance, in both modes.** One-sided poses/exercises (yoga: Warrior I/II, Triangle, Side Angle, Low Lunge, Tree, Eagle, Warrior III, Seated Twist, Cow Face Arms; calisthenics: Reverse Lunge, World's Greatest Stretch, Side Plank, Chair Split Squat, Kneeling Ankle Stretch, and others) are always scheduled as a matched pair, so you never work one side and skip the other. In Yoga, which is timed, the mirrored side is deliberately separated by another pose rather than repeated back-to-back. In Calisthenics, since you're the one deciding when to move on, the mirrored side comes immediately after instead: finish one side, hit Next, and the very next exercise is the other side of the same move (e.g. "Reverse Lunge — Left side" then "Reverse Lunge — Right side"). Not every two-sided move is paired, though. Where it flows better as one continuous set — Bird Dog, Beast Kickthrough, Collarbone Look-Up, Towel Straight-Arm Pull, Couch Stretch, Touch the Potato, Chair Step-Up, and others — it is a single exercise whose cue tells you when to switch sides, which halves how often you have to say "next" to get through it.
- **Calisthenics equipment filter.** Three toggles on the home screen — *I have a mat*, *I have a step, chair, or other prop*, and *I have a pull-up bar* — persisted between sessions. Mat and prop default on; the bar defaults **off**, since a bar is real kit rather than something most rooms happen to contain. With no mat, ground-surface exercises (anything sitting or kneeling on the floor) are excluded entirely; with no prop, anything needing a chair, step, towel, or book is excluded; with no bar, Dead Hang and Pull-Up drop out. A wall is assumed always available, so wall exercises are never filtered. Every entry in `public/js/exercises.js` is flagged accordingly.
...
```js
{
  ...
  mixedSides: true,      // OPTIONAL: both sides mixed within one set (reference shows "Mixed")
...
- **Voice control ("say next")**, Calisthenics mode only, uses [SpeechRecognition](https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition) — solid in Chrome/Edge (desktop and Android) and supported in Safari (as `webkitSpeechRecognition`) since iOS/iPadOS 14.5; Firefox ships it disabled behind a flag. Where it's unsupported the 🎤 button is hidden entirely rather than shown broken — swiping or tapping **Next** always works everywhere, mic or no mic. The first use needs a mic-permission prompt, which is why it starts from a tap; after that the browser remembers the grant for the origin and the app remembers that you had it on, so later Calisthenics sessions arm the mic for you with no tap and no prompt. It auto-restarts itself if the browser silently stops listening after a pause in speech, which several mobile browsers do even in continuous mode.
